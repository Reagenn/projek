        ...
        <div class="card bg-base-100 shadow-sm">
            <div class="card-body">
                ...
                <form id="eventForm" class="space-y-4" method="post"
                    ...
                    <div class="form-control">
                        <label class="label">
                            <span class="label-text font-semibold">Tipe Pembayaran</span>
                        </label>
                        <div class="flex flex-wrap gap-4">
                            @foreach ($paymentTypes as $paymentType)
                            <label class="cursor-pointer label border rounded-lg p-2 hover:bg-base-200">
                                <span class="label-text mr-2">{{ $paymentType->name }}</span>
                                <input type="checkbox" name="payment_types[]" value="{{ $paymentType->id }}" class="checkbox checkbox-primary" 
                                {{ $event->paymentTypes->contains($paymentType->id) ? 'checked' : '' }} />
                            </label>
                            @endforeach
                        </div>
                    </div>
